...
// TestBookCICDStackTrustsMainBranchOnly checks the deploy role only trusts main.
func TestBookCICDStackTrustsMainBranchOnly(t *testing.T) {
	// Arrange
	defer jsii.Close()
				...
				assertions.Match_ObjectLike(&map[string]any{
					...
					"Condition": map[string]any{
						"StringEquals": map[string]any{
							...
							"token.actions.githubusercontent.com:sub": "repo:kazemisoroush/book:ref:refs/heads/main",
						},
					},
				}),
			}),
...
// TestBookCICDStackScopesDeployToBootstrapRoles checks the role only assumes cdk roles.
func TestBookCICDStackScopesDeployToBootstrapRoles(t *testing.T) {
	// Arrange
	defer jsii.Close()
	app := awscdk.NewApp(nil)
	env := &awscdk.Environment{
