...
// newFrontendHosting creates a private S3 bucket served as a static SPA through CloudFront over HTTPS.
func newFrontendHosting(stack awscdk.Stack) frontendHosting {
	...
	// The static export writes one index.html per route. CloudFront only appends the default
	// root object for "/", not for sub-paths, so this viewer-request function rewrites directory
	// and extensionless URIs to their index.html.
...
// deploy uploads the built static site and invalidates the distribution so the new version serves.
func (f frontendHosting) deploy(stack awscdk.Stack) {
...
// URL is the https origin the browser loads.
