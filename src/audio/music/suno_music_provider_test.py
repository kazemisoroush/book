...
def mock_requests():
    ...
    with patch("src.audio.music.suno_music_provider.requests") as mock:
        yield mock
...
def mock_time():
    ...
    with patch("src.audio.music.suno_music_provider.time.sleep"):
        yield
...
def test_suno_cache_hit(tmp_path):
    ...
    # Act
    with patch("src.audio.music.suno_music_provider.requests") as mock_requests:
        result = provider.generate(
            prompt=prompt,
            output_path=output_path,
            duration_seconds=60.0,
...
def test_suno_task_timeout(mock_requests, tmp_path):
    ...
    # Act
    with patch("src.audio.music.suno_music_provider.time.sleep"):
        with patch("src.audio.music.suno_music_provider.time.time") as mock_time:
