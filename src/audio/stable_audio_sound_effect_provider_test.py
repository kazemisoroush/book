...
from src.audio.stable_audio_sound_effect_provider import StableAudioSoundEffectProvider


@pytest.fixture
def mock_requests():
    """Mock requests module."""
    with patch("src.audio.stable_audio_sound_effect_provider.requests") as mock:
        yield mock
...
def test_stable_audio_sfx_generate_success(mock_requests, tmp_path):
    ...
    provider = StableAudioSoundEffectProvider(api_key="test-key", cache_dir=cache_dir)
...
def test_stable_audio_sfx_cache_hit(tmp_path):
    ...
    # Act
    with patch("src.audio.stable_audio_sound_effect_provider.requests") as mock_requests:
        result = provider.generate(
...
def test_stable_audio_sfx_empty_api_key_raises_valueerror(tmp_path):
    ...
    with pytest.raises(ValueError, match="API key cannot be empty"):
        StableAudioSoundEffectProvider(api_key="", cache_dir=tmp_path)
