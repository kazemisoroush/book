...
class TestProjectGutenbergWorkflowFactory:
    ...
    def test_create_without_section_parser(self):
        # When
        workflow = ProjectGutenbergWorkflow.create(with_section_parser=False)
...
class TestProjectGutenbergWorkflow:
    ...
    def test_run_segments_sections_when_section_parser_provided(self):
        # Given
        mock_downloader = Mock()
        mock_downloader.parse.return_value = True
        mock_downloader._extract_book_id.return_value = "123"
        ...
        mock_content_parser.parse.return_value = BookContent(
            chapters=[
                Chapter(number=1, title="Chapter 1", sections=[section1, section2])  # noqa: E501
            ]
        )
        ...
        mock_section_parser.parse.side_effect = [
            [
                Segment(text="Hello", segment_type=SegmentType.DIALOGUE, speaker="Tom"),  # noqa: E501
                Segment(text="said Tom.", segment_type=SegmentType.NARRATION)
            ],
            [
                Segment(text="It was a sunny day.", segment_type=SegmentType.NARRATION)  # noqa: E501
            ]
        ]
        ...
        assert book.content.chapters[0].sections[0].segments[0].text == "Hello"
        assert book.content.chapters[0].sections[0].segments[0].speaker == "Tom"  # noqa: E501
        assert len(book.content.chapters[0].sections[1].segments) == 1
        assert mock_section_parser.parse.call_count == 2

    def test_run_skips_segmentation_when_no_section_parser(self):
        # Given
        mock_downloader = Mock()
        mock_downloader.parse.return_value = True
        mock_downloader._extract_book_id.return_value = "123"
        ...
        section1 = Section(text='Some text.')
        mock_content_parser = Mock()
        mock_content_parser.parse.return_value = BookContent(
            chapters=[
        ...
        workflow = ProjectGutenbergWorkflow(
            ...
            mock_metadata_parser,
            mock_content_parser,
            section_parser=None  # No section parser
        )
