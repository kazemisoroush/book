...
`save(book)` / `save_chapter(book, chapter)` / `load(book_id)` / `exists(book_id)` plus `save_input(book)` / `load_input(book_id)` for the pre-AI snapshot. Abstract so the storage backend can be swapped (filesystem today, database later) without changing callers. `save()` derives the id from `Book.book_id`; `load()` / `exists()` take a `book_id` directly so callers can probe the cache before having a `Book`. `save_chapter(book, chapter)` is called once per parsed chapter by [AIWorkflow](../workflows/ai_workflow.py); backends with full-book state delegate to `save(book)`, backends with per-chapter content (e.g. ElevenLabs Studio) use the chapter as the unit of update.

Every workflow now accepts a `repositories: list[BookRepository]`. Reads use `repositories[0]`; writes fan out to every repository in the list. The factory wires `[FileBookRepository, StudioBookRepository]` by default when `ELEVENLABS_API_KEY` is set.
...
### StudioBookRepository

Mirrors each parsed chapter into an [ElevenLabs Studio](https://elevenlabs.io/docs/api-reference/studio) project. `save_chapter(book, chapter)` upserts a project named `book.book_id` and a chapter named `Chapter {N}`, then replaces the chapter's blocks with one `tts_node` per narratable beat. The voice for each node is `book.voice_assignments.get(beat.character_id, default_voice_id)` so the AI workflow can push to Studio before the characters workflow runs; the `default_voice_id` is constructor-supplied (and configurable via `ELEVENLABS_STUDIO_DEFAULT_VOICE_ID`). `save`, `save_input`, `load`, `load_input`, and `exists` are no-ops, so pairing this repo with `FileBookRepository` keeps cache-resume working.
