...
The `claude-code` ai provider runs the workflow through the Claude Code CLI in print mode (`claude --print`), so calls bill against the signed-in claude.ai Pro/Max plan instead of an API key or AWS Bedrock. It requires Claude Code installed on the host and signed in. It has no Anthropic-style prompt caching at the wire level. Pro/Max quota throttles long full-book runs.
