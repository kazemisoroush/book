...
def test_prompt_includes_book_start_hint_when_is_book_start_true():
    """When is_book_start=True, the dynamic portion should tell the LLM this is the start of the book."""
    # Arrange
    builder = PromptBuilder(book_title="Pride and Prejudice")
    registry = CharacterRegistry.with_default_narrator()
    ...
    prompt = builder.build_prompt(
        ...
        registry, None, scene_registry=None,
        is_book_start=True,
    )

    # Assert
    dynamic = prompt.build_dynamic_portion()
    assert "start of the book" in dynamic.lower()


def test_prompt_includes_chapter_start_hint_when_is_chapter_start_true():
    """When is_chapter_start=True, the dynamic portion should tell the LLM this is the start of a chapter."""
    # Arrange
    builder = PromptBuilder()
    registry = CharacterRegistry.with_default_narrator()
    ...
    prompt = builder.build_prompt(
        ...
        registry, None, scene_registry=None,
        is_chapter_start=True, chapter_title="The Arrival",
    )

    # Assert
    dynamic = prompt.build_dynamic_portion()
    assert "start of" in dynamic.lower()
    assert "chapter" in dynamic.lower()


def test_prompt_includes_chapter_title_in_hint_when_provided():
    """When chapter_title is provided with is_chapter_start, the hint should include the title."""
    # Arrange
    builder = PromptBuilder()
    registry = CharacterRegistry.with_default_narrator()
    ...
    # Assert
    dynamic = prompt.build_dynamic_portion()
    assert "The Arrival" in dynamic


def test_prompt_no_position_hint_when_flags_are_false():
    """When both position flags are False (default), no position hint should appear."""
    # Arrange
    builder = PromptBuilder()
    registry = CharacterRegistry.with_default_narrator()
    ...
    # Act
    prompt = builder.build_prompt(
        "Some middle-of-chapter text.",
        registry, None, scene_registry=None,
    )

    # Assert
    dynamic = prompt.build_dynamic_portion()
    assert "start of the book" not in dynamic.lower()
    assert "Section position" not in dynamic
