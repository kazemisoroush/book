# TD-015 — Replace AudioOrchestrator Class Constants with Feature Flags

## Goal

Replace hardcoded class constants in `AudioOrchestrator` with a `FeatureFlags`
...
`AudioOrchestrator` constructor. Read flag values from it instead of class
...
1. `AudioOrchestrator` accepts a `FeatureFlags` instance at construction.
2. All class-level feature constants are read from the flags object.
3. Behaviour is unchanged with default flag values.
4. New flags can be added without modifying `AudioOrchestrator` source.
