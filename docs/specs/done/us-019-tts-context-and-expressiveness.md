...
1. **Character voices sound robotic** — each beat is synthesized as a cold
...
- **`previous_text`**: text that came before the current beat. Helps the
  model match prosody — how to *begin* the beat given what preceded it.
- **`next_text`**: text that comes after. Helps the model know how to *end*
  the beat — whether the thought continues or terminates.

This directly fixes problems #1 and #3. Each beat is no longer a cold
...
| `src/audio/audio_orchestrator.py` | In `_synthesise_beats`, look up adjacent beat text and pass it |
...
**Caveat**: only works for consecutive beats using the **same voice**.
...
### Fix 4 — Richer expressiveness via aggressive beat splitting (medium impact)
...
Each sub-beat gets its own voice settings (stability/style/speed) from
the LLM (Fix 3), and Fixes 1+2 smooth transitions between beats via
`previous_text`/`next_text` and `previous_request_ids`.

ElevenLabs bills per character, not per API call, so more beats do not
increase cost.
...
beat need?" should be extracted from `AudioOrchestrator._synthesise_beats`
into a dedicated `BeatContextResolver` (or similar). This keeps the
