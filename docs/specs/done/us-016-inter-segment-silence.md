# US-016 — Inter-Beat Silence

## Goal

Insert a configurable silence gap between consecutive TTS beats when
stitching audio, so the assembled chapter breathes naturally instead of
running words together at beat boundaries.

---

## Background / motivation

Beats are currently concatenated with ffmpeg using a plain `concat`
...
- **Same-speaker boundary** (narration block continues, or the same
  character speaks in two adjacent beats): short gap (~150 ms).
...
4. Beat boundary type is determined by comparing `beat.character_id`
   of adjacent beats:
...
5. The `_stitch_with_ffmpeg` method signature and call sites are updated
   to accept the beat list (needed to determine boundary types).

6. Unit tests cover:
   - Same-speaker boundary uses short silence duration.
   - Speaker-change boundary uses long silence duration.
   - Silence clips are inserted between every pair (N beats → N-1 gaps).
   - Single-beat chapter produces no silence clip.

---

## Out of scope

- Silence after the final beat.
- Per-emotion gap tuning.
- Silence within a beat (see US-017).
...
minimum needed to feel natural without requiring per-beat tuning.
